...
class ReservaListCreateView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Validamos que no exista una reserva igual (opcional)
        if Reserva.objects.filter(usuario_id=usuario_id, pelicula_id=pelicula_id).exists():
            return Response(
                {"error": "Esta reserva ya fue realizada."},
